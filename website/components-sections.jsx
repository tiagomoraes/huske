...
const RELEASES = [
  {
    ...
    items: [
      { kind: "changed", text: <><strong>Ruff and mypy are CI gates now</strong>, not conventions. Both were already clean; they were just never enforced, so "clean" depended on whoever remembered to run them. The new <code>Lint & types</code> job runs <code>ruff check .</code> and <code>mypy huske</code>. The two <code>@mcp.tool()</code> ignores gained <code>unused-ignore</code> because CI installs only <code>.[dev]</code> — without the <code>mcp</code> extra the decorator reads as untyped and the ignore is required, while a machine with the extra sees it as redundant.</> },
      { kind: "changed", text: <><code>scripts/update-homebrew-tap.py</code> retries the pip resolve before giving up. It runs minutes after the release publishes, and PyPI's JSON API, simple index and file CDN go live at different times — v0.11.1 failed once and succeeded a minute later, unchanged. Four attempts, 20 s apart, then a message that names propagation as the likely cause.</> },
      { kind: "changed", text: <><code>actions/upload-artifact</code> in CI matches the release workflow (v7); it had been left on v4.</> },
      { kind: "fixed", text: <><strong>A latent unpack bug in the built-in distillation backend.</strong> <code>mlx_lm.load()</code> is typed as returning either <code>(model, tokenizer)</code> or <code>(model, tokenizer, config)</code> and is not overloaded, so the two-value unpack could not be narrowed. Runtime was fine — we never pass <code>return_config</code> — but the code was one keyword argument away from breaking, and no local run caught it because an interpreter without <code>mlx-lm</code> sees the whole module as <code>Any</code>. The new mypy gate found it on its first CI run.</> },
      { kind: "added", text: <><strong>huske auto-manages the local Ollama daemon for distillation.</strong> Only relevant on <code>distill_backend = "ollama"</code> — the default <code>mlx</code> backend downloads its own model. When distillation turns on, at launch or via the app / menu-bar toggle, huske now starts <code>ollama serve</code> if the <code>ollama</code> CLI is installed but idle, and pulls the configured model if it's missing (streaming progress to the events log), instead of only warning that the daemon is unreachable. It only ever runs the local <code>ollama</code> CLI — it never installs Ollama — and still degrades to the same actionable hint when it can't help. Opt out with <code>distill_auto_manage = false</code> or <code>--no-distill-auto-manage</code>. The model it starts and pulls is the existing, editable <code>distill_model</code>.</> },
    ],
  },
  {
