...
from huske.mcp.tools import UnknownPassageError, fetch_passage, search_passages
...
def build_server(
    ...
    *,
    ...
    """Build a FastMCP server exposing ``search`` and ``fetch`` over ``store``."""
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "passages by meaning, optionally filtered by date range, audio source "
            "(mic = the user; system = the other party), or session; then `fetch` "
            "a passage id for its full text and citation metadata."
        ),
        host=host,
        port=port,
    ...
    def fetch(id: str, context: int = 0) -> dict[str, Any]:
        """Fetch one transcript passage by id, with citation metadata.

        Args:
            id: A passage id returned by ``search``.
            context: Number of neighboring passages to include on each side.
        """
        try:
            return fetch_passage(store, id, context=context)
...
def _print_banner(host: str, port: int, token: str, stats: dict[str, object]) -> None:
...
def run(
    ...
    *,
    ...
    server = build_server(store, embedder, host=host, port=port)
    app = BearerAuthMiddleware(server.streamable_http_app(), token)
    _print_banner(host, port, token, store.stats())
